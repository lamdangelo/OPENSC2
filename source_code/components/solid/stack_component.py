...
from components.solid.stack_component_inputs import StackComponentInputs
...
class StackComponent(StrandComponent):
    ...
    def __init__(
        ...
        Args:
            ...
            name (str): component kind string (e.g. "STACK", "STR_SC").
            inputs (StackComponentInputs): pre-parsed input parameters.
        ...
        hts_layer = next(layer for layer in self.inputs.tape_layers if layer.is_hts)
        self.sc = (
            hts_layer.thickness
    ...
    def __reorganize_input(self):
        ...
        all_materials = np.array(
            [layer.material for layer in self.inputs.tape_layers], dtype=str
        )
        self.__index_material_none = np.nonzero(all_materials == "none")[0]
        self.tape_material = all_materials[np.nonzero(all_materials != "none")[0]]

        self.tape_material_not_sc = np.array(
            [layer.material for layer in self.inputs.tape_layers
             if not layer.is_hts and layer.material != "none"],
            dtype=str,
        )

        all_thicknesses = np.array(
            [layer.thickness for layer in self.inputs.tape_layers], dtype=float
        )
        self.__index_thickness_0 = np.nonzero(all_thicknesses == 0)[0]
        self.material_thickness = all_thicknesses[np.nonzero(all_thicknesses)[0]]

        self.tape_thickness = self.material_thickness.sum()

        self.material_thickness_not_sc = np.array(
            [layer.thickness for layer in self.inputs.tape_layers
             if not layer.is_hts and layer.thickness > 0.0],
            dtype=float,
        )
    ...
    def __check_consistency(self, conductor):
        ...
        if self.tape_material.size != self.inputs.num_material_layers:
        ...
        if self.material_thickness.size != self.inputs.num_material_layers:
            # Message to be completed!
            raise ValueError(
        ...
        # evaluated one.
        if self.__tape_number != self.inputs.num_tapes:
            # Message to be completed!
            raise ValueError(
    ...
    def stack_thermal_conductivity(self, property: dict) -> np.ndarray:
        ...
        thermal_conductivity = np.zeros(
            (property.temperature.size, self.inputs.num_material_layers)
        )
    ...
    def stack_electrical_resistivity_not_sc(self, property: dict) -> np.ndarray:
        ...
        electrical_resistivity = np.zeros(
            (property.temperature.size, self.inputs.num_material_layers - 1)
        )
        ...
        if self.inputs.num_material_layers - 1 > 1:
        ...
        elif self.inputs.num_material_layers - 1 == 1:
    ...
    def get_electric_resistance(self, conductor: object) -> np.ndarray:
        ...
        critical_current_node = self.sc_cross_section * self.node_fields.J_critical
        critical_current_gauss = (
            self.sc_cross_section * self.gauss_fields.J_critical
        )
        ...
        self.node_fields.electric_resistance = 10.0 * np.ones(
            self.node_fields.temperature.shape
        )
        self.gauss_fields.electric_resistance = 10.0 * np.ones(
            self.gauss_fields.temperature.shape
        )
        ...
        self.node_fields.electrical_resistivity_superconductor = np.full_like(
            self.node_fields.temperature, None
        )
