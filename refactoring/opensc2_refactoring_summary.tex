...
\begin{abstract}
This document summarizes the restructuring of the OPENSC2 code base (a coupled
...
of the wall time for the full \SI{25}{s} transient from roughly four hours to
\textbf{19 minutes}, on par with the commercial reference code CryoSoft THEA
($\approx$15 minutes for the equivalent configuration), while every individual
optimization step was verified to be bit-compatible (relative deviations
$\lesssim 10^{-12}$, i.e.\ floating-point summation-order noise) against a frozen
solver state, and the physical results reproduce the THEA reference solution.
\end{abstract}
...
(commit \code{dfeda15}) in June~2026. At that point OPENSC2 consisted of a flat
...
All optimizations were verified against frozen 30-step solver-state dumps of the
W7-X case; the accepted tolerance was floating-point summation-order noise
...
LAPACK via \code{scipy.linalg.solve\_banded}, including a documented remapping between
...
\begin{enumerate}
  ...
  \item \textbf{Physical benchmark} (final phase): the W7-X quench case reproduces
        the CryoSoft THEA reference solution (hot-spot trajectory within
...
\begin{itemize}
  ...
  \item \textbf{Regression suite}: the TDD example cases should be promoted into an
        automated regression harness now that the code runs end-to-end.
