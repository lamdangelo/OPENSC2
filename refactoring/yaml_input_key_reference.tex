...
\title{OPENSC2 YAML Input Format:\\Key Reference (Schema v2)}
...
\code{outlet\_heated\_zone\_start/\allowbreak end} & XJBEOUT, XJENOUT & float, m \\
...
\code{external\_free\_convection\_correlation} & (unchanged) & correlation name string \\
\bottomrule
\end{longtable}

\section*{conductor.operations}

\begin{longtable}{p{5.4cm}p{4.6cm}p{5.2cm}}
\toprule
\textbf{v2 key} & \textbf{Excel name} & \textbf{Type / values} \\
\midrule
\endhead
...
\code{magnetic\_field\_inlet/outlet\_initial} & BISS, BOSS & float, T \\
\code{magnetic\_field\_inlet/outlet\_transient} & BITR, BOTR & float, T \\
...
\code{current\_sharing\_temperature\_evaluation} & TCS\_EVALUATION & boolean; evaluate $T_{cs}$ every step \\
\code{fixed\_potential\_flag/number} & FIX\_POTENTIAL\_FLAG/NUMBER & boolean; int \\
\code{fixed\_potential\_coordinates/values} & FIX\_POTENTIAL\_COORDINATE/VALUE & comma-separated floats (m; V) \\
\bottomrule
\end{longtable}
...
\begin{longtable}{p{5.8cm}p{3.2cm}p{6.2cm}}
\toprule
\textbf{v2 key} & \textbf{Excel name} & \textbf{Type / values} \\
\midrule
\endhead
...
\code{stabilizer\_strand\_count/diameter} & N\_stab\_strand, d\_stab\_strand & int; float, m \\
\code{stabilizer\_to\_superconductor\_ratio(\_mode)} & STAB\_NON\_STAB, ISTAB\_NON\_STAB & float; int flag \\
